...
class Vilib(object): 
    ...
    # stf_flag = False
    video_source = 0
    roi = cv2.imread("/opt/ezblock/cali.webp")
    roi_hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)

    obj_roi = cv2.imread("/opt/ezblock/object.webp")
    h,w = obj_roi.shape[:2]
...
#picture
    ...
    detect_obj_parameter['picture_path'] = '/home/pi/picture_file/' + time.strftime("%Y-%m-%d-%H-%M-%S")+ '.webp'
    # detect_obj_parameter['color_default'] = 'red'
...
### video
    ...
    def gesture_calibrate(img):
        if Vilib.detect_obj_parameter['calibrate_flag'] == True:
            # cv2.VideoWriter("./video_file/tt.avi", fourcc, 20.0, (640, 480))
                # roi_hsv = roi_hsv
            cv2.imwrite('/opt/ezblock/cali.webp', img[90:150,130:190])
    ...
    def get_picture(process_picture):
        ...
        # if Vilib.detect_obj_parameter['picture_flag'] == True:
        # Vilib.detect_obj_parameter['picture_path'] = '/home/pi/picture_file/' + time.strftime("%Y-%m-%d-%H-%M-%S") + '.webp'
        Vilib.detect_obj_parameter['picture_path'] = '/home/pi/picture_file/' + time.strftime("%Y-%m-%d-%H-%M-%S") + '.webp'
            # cv2.VideoWriter("./video_file/tt.avi", fourcc, 20.0, (640, 480))
            # cv2.imwrite(pic_path, Vilib.img_array[0])

...
traffic_sign_model_path = "./traffic_sign_model_100.tflite"
gesture_model_path = "./mobile_96_0.35_15.tflite"
...
def gen():
    ...
    while True:  
        ...
        # print("1")
        # if Vilib.conn2.recv()
        # frame = cv2.imencode('.webp', Vilib.conn2.recv())[1].tobytes() 
        # rt_img = np.ones((320,240),np.uint8)
        # print("2")

        frame = cv2.imencode('.webp', Vilib.img_array[0])[1].tobytes()
        yield (b'--frame\r\n'
...
#picture
    ...
    detect_obj_parameter['picture_path'] = './picture_file/' + time.strftime("%Y-%m-%d-%H-%M-%S")+ '.webp'
    # detect_obj_parameter['color_default'] = 'red'
...
### video
    ...
    def get_picture(process_picture):
        ...
        # if Vilib.detect_obj_parameter['picture_flag'] == True:
        Vilib.detect_obj_parameter['picture_path'] = './picture_file/' + time.strftime("%Y-%m-%d-%H-%M-%S") + '.webp'
    ...
    def traffic_predict(input_img,x,y,w,h):
        ...
        # cv2.imwrite(str(x)+str(y)+'.webp',new_img)
        new_img = (new_img / 255.0)
        # img = img / 255.
        # new_img = (new_img - 0.5) * 2.0

        resize_img = cv2.resize(new_img, (32,32), interpolation=cv2.INTER_LINEAR)
        flatten_img = np.reshape(resize_img, (32,32,3))
...
### detection
    ...
    def traffic_detect(img):
        ...
        if Vilib.detect_obj_parameter['ts_flag']  == True:
            ...
            ### red
            mask_red = cv2.inRange(hsv,(166,50,20), (180,255,255))
            ...
            # print(Vilib.lower_color)
                                ...
                                if circles is not None:
                                    ...
                                    # cv2.rectangle(img,(x,y),(x+w,y+h),(0,255,0),2) 
                                        ...
                                        cv2.putText(img,str(traffic_dict[traffic_type]) +': ' + str(round(acc_val*100)),(int(x+i[0]-i[2]),int(y+i[1]-i[2])), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,0,255),2)#加减10是调整字符位置
                                        if w * h > max_area:
                                            max_area = w * h
                                            max_obj_x = x
                                            max_obj_y = y
                                ...
                                #     cv2.drawContours(img,blue_contours,0,(0,0,255),1)
                                    ...
                                    if corners == 8:
                                        ...
                                        cv2.putText(img,str(traffic_dict[traffic_type]) +': ' + str(round(acc_val*100)),(x,y), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,0,255),2)#加减10是调整字符位置
                                        if w * h > max_area:
                                            max_area = w * h
                                            max_obj_x = x
                                            max_obj_y = y
                ...
                if traffic_sign_num > 0:
                    ...
                    Vilib.detect_obj_parameter['traffic_sign_t'] = max_obj_t
    ...
    ###肤色部分
            ...
            if ges_num > 0:
                ...
                if w >= 60 and h >= 60:
                    ...
                    acc_val,ges_type = Vilib.gesture_predict(img,x-0.2*w,y-1*h,1.2*w,2*h) 
                        # x = x*2
                        # y = y*2
                        # w = w*2
                        # h = h*2
                    ...
                    if acc_val >= 65:
                        ...
                        cv2.putText(img,ges_dict[ges_type]+': '+str(acc_val) + '%',(105,17),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,255,255),2)  ##(0,97,240)
